...
\begin{enumerate}
    ...
    \item $\mathbb{P}(A_i) > 0$ \quad для всех $i = 1, \ldots, n$ (\textit{нетривиальны}).
\end{enumerate}
...
\textbf{Формулы Байеса.} Пусть даны события $A, H_1, \ldots, H_n, \ldots;$ $\mathbb{P}(A) > 0$, $\mathbb{P}(H_i) > 0$, причём $H_i \cap H_j = \varnothing~(i \ne j)$ и $\bigcup\limits_{i=1}^\infty H_i \supset A$ (например, $\bigcup\limits_{i=1}^\infty H_i = \Omega$).
