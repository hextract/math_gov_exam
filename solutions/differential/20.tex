...
\begin{align*}
  ...
  = \sum_{s=1}^m \left(\overline{c}_{2s-1} e^{\lambda_{2s} x} + \overline{c}_{2s} e^{\lambda_{2s-1} x}\right) + \sum_{s=2m+1}^n \overline{c}_s e^{\lambda_s x}
\end{align*}
...
\begin{enumerate}
  \item Возьмем базис $e_j(x) = \frac{e^{\mu x} x^{j-1}}{(j-1)!}, j=1, \dots, m+s $ в $Q_{\mu, m+s}$.
    \begin{align*}
      D(e_1(x)) &= \mu e_1(x),\\
