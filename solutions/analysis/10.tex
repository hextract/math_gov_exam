...
\Theor{Необходимое условие интегрируемости} $f \in R([a, b]) \;\Rightarrow\; f \in B([a, b])$ (ограничена на этом отрезке).
...
\Def Для разбиения $T$ отрезка $[a, b]$ выборкой назовем конечный набор точек $\{\xi_i\}_{i=1}^{N_T}$, такой что $\forall i \in \{1, \dots, N_T\} \hookrightarrow \xi_i
\in [x_{i-1}, x_i]$.

Интегральная сумма Римана: $\Sigma(f, T, \xi_T) = \sum_{i=1}^N f(\xi_i) (x_i - x_{i-1})$.
...
\begin{enumerate}
  \item \textbf{Производная} функции $F(x) =
    \begin{cases*}
...
\begin{gather*}
  \left|\frac{F(x) - F(x^0)}{x - x^0} - f(x^0)\right| = \left|\frac{\int_x^{x^0} f(t) dt - f(x^0) \cdot (x - x^0)}{x - x^0}\right| =
...
Итого, $\forall \eps > 0 \;\exists \delta > 0 \;\forall x \in \mathring U_\delta (x^0) \cap [a, b] \hookrightarrow \left|\frac{F(x) - F(x^0)}{x - x^0} - f(x^0)\right| <
\eps$, что по определению значит, что $\exists F'(x^0) = f(x^0)$ для произвольной точки $x^0 \in (a, b)$.
