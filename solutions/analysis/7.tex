...
Раз это верно для любого $\delta$, то построим последовательности $\{x_k\}, \{y_k\}$, такие что $\rho_X(x_k, y_k) < \frac{1}{k}$ и $\rho_Y(f(x_k), f(y_k)) \geq \eps$ для $\forall k \in \N$.

Раз $A$ -- компакт, из $\{x_k\}$ можно выделить сходящуюся в $A$ подпоследовательность: $x_{k_j} \to c \in A$. 

А так как расстояние между соответствующими членами подпоследовательностей стремится к 0, то, воспользовавшись неравенством треугольника, получаем, что соответствующие подпоследовательности стремятся к одной точке: 
