...
\Def Пусть $n \in \N, f: U_{\delta_0} (x_0) \to \R$ имеет $n$-ую конечную производную в точке $x_0$. Тогда полиномом Тейлора функции $f$с центром в точке $x_0$ будем называеть следующую функцию:
\begin{equation*}
...
Заметим, что коль скоро $f$ $n$ раз диффернцируема в точке $x_0$, то она $(n-1)$-раз диффернцируема в некоторой её окрестности (в частности она непрерывна на замыкании этой окрестности).
...
Даоее аналогично перепишем результат выше:
\begin{equation*}
...
\Theor{Формула Тейлора с остаточным членом в форме Лангранжа} Пусть $\exists f^{(n+1)}(x), \forall x \in U_\delta(x_0)$, тогда
\begin{equation*}
