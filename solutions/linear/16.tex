...
\begin{itemize}
  ...
  \item $\exists 0 \in \mathcal{L}: \forall x \in \mathcal{L} \hookrightarrow x + 0 = 0 + x = x$
...
Далее:
\begin{equation*}
  \forall \alpha, \beta \in \R, \forall x, y \in \mathcal{L}' \hookrightarrow \alpha\varphi(x) + \beta\varphi(y) = \varphi(\alpha x + \beta y) \in \mathcal{L}_2
\end{equation*}
...
Пусть $e_1, \dots e_n$ - базис $\mathcal{L}'$. Пусть $x \in \mathcal{L}' \Rightarrow \varphi(x) = x_1 \varphi(e_1) + \dots + x_n \varphi(e_n) \Rightarrow \dim
\varphi(\mathcal{L}') \leq \dim \mathcal{L}'$.
