...
\Def В многомерном случае $\theta \in \Theta \subset \mathbb{R}^k$ множество $S(X) \subset \Theta$ называется \textit{доверительным множеством} (или доверительной областью) уровня $\gamma$, если
\[
...
\Def Пусть $(X_n)_{n \geq 1}$ — выборка неограниченного объёма из распределения $P_\theta$. Последовательность пар статистик $(T_1^{(n)}, T_2^{(n)})$ называется \textit{асимптотическим доверительным интервалом} уровня $\gamma$, если
\[
...
\[
\theta \in \left(\hat{\theta}_n - u_{\frac{1+\gamma}{2}} \frac{\sigma(\hat{\theta}_n)}{\sqrt{n}}, \quad \hat{\theta}_n + u_{\frac{1+\gamma}{2}} \frac{\sigma(\hat{\theta}_n)}{\sqrt{n}}\right).
\]
