...
Пусть $\hat{\theta}_{ML}$ — ОМП параметра $\theta$, и $g: \Theta \to \mathbb{R}$ — произвольная функция. Тогда $g(\hat{\theta}_{ML})$ — ОМП параметра $g(\theta)$.

\Proof

Пусть $\eta = g(\theta)$. Для любого $\eta$ из области значений $g$:
\[
\sup_{\theta: g(\theta) = \eta} L(\theta; X) \leq \sup_{\theta \in \Theta} L(\theta; X) = L(\hat{\theta}_{ML}; X).
\]

Максимум функции правдоподобия по $\eta$ достигается при $\eta = g(\hat{\theta}_{ML})$.

\Endproof

\textbf{Замечание.} Инвариантность — важное свойство ОМП. Например, если $\hat{\sigma}^2$ — ОМП для дисперсии, то $\hat{\sigma} = \sqrt{\hat{\sigma}^2}$ — ОМП для стандартного отклонения.
