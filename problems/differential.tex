...
\begin{align*}
  \lambda_1 = 2-\sqrt{2}: & \quad v_1 =
  \begin{pmatrix} 1 \\ -\sqrt{2} \\ 1
  \end{pmatrix} \\
  \lambda_2 = 2: & \quad v_2 =
  \begin{pmatrix} 1 \\ 0 \\ -1
  \end{pmatrix} \\
  \lambda_3 = 2+\sqrt{2}: & \quad v_3 =
  \begin{pmatrix} 1 \\ \sqrt{2} \\ 1
  \end{pmatrix}
\end{align*}
...
\begin{equation*}
  \begin{pmatrix} x(t) \\ y(t) \\ z(t)
  \end{pmatrix} =
  C_1
  \begin{pmatrix} 1 \\ -\sqrt{2} \\ 1
  \end{pmatrix} e^{(2-\sqrt{2})t} +
  C_2
  \begin{pmatrix} 1 \\ 0 \\ -1
  \end{pmatrix} e^{2t} +
  C_3
...
Для \(\lambda = i\) находим собственный вектор \(v_1 =
  \begin{pmatrix} 1 \\ -i
\end{pmatrix}\), для \(\lambda = -i\) — \(v_2 =
  \begin{pmatrix} 1 \\ i
\end{pmatrix}\).
...
\begin{equation*}
  (A - 0 I)v_1 = 0 \quad \Rightarrow \quad v_1 =
  \begin{pmatrix} 1 \\ -2
  \end{pmatrix}
\end{equation*}
...
\begin{equation*}
  (A - 0 I)v_2 = v_1 \quad \Rightarrow \quad v_2 =
  \begin{pmatrix} 0 \\ 1
  \end{pmatrix}
\end{equation*}
...
\Note
Если система $X' = A X$ имеет вещественный собственный корень \(\lambda\) кратности \(k\), а собственный вектор всего 1, то необходимо строить цепочку обобщённых
...
\begin{equation*}
  X(t) = \Big(C_1 v_1 + C_2 (v_1 t + v_2) + C_3 \big(v_1 \frac{t^2}{2} + v_2 t + v_3\big) + \dots + C_k \big(v_1 \frac{t^{k-1}}{(k-1)!} + v_2 \frac{t^{k-2}}{(k-2)!} +
  \dots + v_k\big)\Big) e^{\lambda t},
\end{equation*}
...
\[
  \det(A - \lambda I) =
  \begin{vmatrix} 4 - \lambda & 3 \\ 3 & 4 - \lambda
  \end{vmatrix}
